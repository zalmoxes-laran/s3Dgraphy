...
def test_partial_unique_index_allows_null_collision(clean_pg_with_seed):
    ...
    """
    from s3dgraphy.sync._db_handle import DbHandle
    from tests.sync._uuid_backfill import add_columns
    handle = DbHandle.from_engine(clean_pg_with_seed,
                                   str(clean_pg_with_seed.url))
    add_columns(handle)
...
def test_atomic_rollback_on_alter_failure(clean_pg_with_seed, monkeypatch):
    ...
    from s3dgraphy.sync._db_handle import DbHandle, _columns_of
    from tests.sync import _uuid_backfill as lib
...
@pytest.mark.skip(reason=(
    "pg_dump auto-backup helper lives in pyArchInit's "
    "scripts/migrations/_common.py — not part of s3dgraphy.sync. "
    "Covered by pyarchinit/tests/sync/test_node_uuid_backfill_pg.py "
    "in the shim repo."
))

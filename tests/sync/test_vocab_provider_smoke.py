"""Smoke test against the real bundled JSON_config/ shipped inside s3dgraphy.

Skipped if the package's JSON_config/ cannot be located (e.g. an
installed wheel that omitted the data directory).
"""
...
BUNDLED = Path(s3dgraphy.__file__).resolve().parent / "JSON_config"


@pytest.mark.skipif(not BUNDLED.is_dir(),
                    reason="s3dgraphy/JSON_config/ not present")
...
def test_real_bundled_versions_present(tmp_path):
    core = VocabProviderCore(bundled_dir=BUNDLED, overrides_dir=tmp_path)
    v = core.versions
    # All three pillars must report a version on the real vendor.
    assert v.nodes
